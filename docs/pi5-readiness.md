...
1. **DSI lid + HDMI deck (leading option).** The deck runs a small HDMI LCD as a Chromium `/panel` kiosk — keeps the web-UI authoring + three.js/Svelte dial animations the deck is for. HDMI is plug-and-play with a huge panel selection, and **avoids the dual-DSI 3.3V-rail risk entirely** (only one DSI). Coexistence is fine: DSI + HDMI are independent DRM cards and **labwc or X11 drive both** (only **Wayfire** can't — it renders to one display; don't use it). **Two caveats to verify on arrival:**
   - **Current kernel HDMI regression.** A Feb-2026 regression (kernel `6.12.62+rpt` on RPi OS Trixie) breaks HDMI output on Pi 4/5; working kernels are **6.6.x (Bookworm)** / 6.1.x. So today HDMI means pinning to the Bookworm kernel or waiting for the fix — likely resolved by the time the deck phase lands, but **test HDMI on the actual kernel** before committing. ([forum](https://forums.raspberrypi.com/viewtopic.php?p=2364475))
...
6. **HDMI deck (leading topology): #7041 is closed/resolved and DSI+HDMI coexist under labwc/X11** — but **don't use Wayfire** (renders to one display), and the live risk is the **Feb-2026 6.12.x HDMI kernel regression** (pin 6.6.x / verify fixed on the box). If you instead go dual-DSI, #7041 is irrelevant (no HDMI) and the 3.3V rail is the risk. Test whichever topology on the real boards + kernel.
...
1. **First — the bot on the Pi (no screens at all).** This is the risk-free win and the thing to do as soon as the Pi arrives: headless migration per next.md first-boot + deploy.md Phase 1, **run the agent-browser screenshot path by hand** to validate the arm64 Chromium before trusting 06:45, enable **zram** (`apt install zram-tools`, `vm.swappiness=10`), parallel-run beside the X230, then move the MUNBYN cable over. Needs no desktop, no screens — pure server. Buy the **64 GB A2 SD card** (above) for this.
2. **Lid screen next (single DSI — zero topology risk).** Buy the **10.1" DSI (C)** + **one** 22→15-pin cable; install the kiosk stack as an apt layer (labwc + Chromium user service); show `/clock`; wire the **backlight-schedule job** (discover the real `/sys/class/backlight` node first). One DSI panel = no dual-DSI rail risk and no HDMI question yet.
3. **Decide the deck topology — only after testing on the box.** With the lid DSI already up, plug in a **small HDMI LCD** and confirm: (a) the box's kernel drives HDMI (avoid the 6.12.x regression — pin 6.6.x if needed), (b) DSI + HDMI coexist under **labwc or X11** (not Wayfire), (c) the panel blanks acceptably when idle. If HDMI behaves → buy that panel for the deck (leading path). If not → fall back to a second DSI (5" B Rev2.2, accept the rail test) or a Pico-driven deck. **Don't buy the deck panel until this test passes.**
