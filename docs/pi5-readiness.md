...
3. **Decide the deck topology — only after testing on the box.** With the lid DSI already up, plug in a **small HDMI LCD** and confirm: (a) the box's kernel drives HDMI (the 6.12 regression is superseded by 6.18.34 — just smoke-test), (b) DSI + HDMI coexist under **labwc or X11** (not Wayfire), (c) the panel blanks acceptably when idle. If HDMI behaves → buy that panel for the deck (leading path). If not → fall back to a second DSI (5" B Rev2.2, accept the rail test) or a Pico-driven deck. **Don't buy the deck panel until this test passes.**
