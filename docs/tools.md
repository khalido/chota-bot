...
Tools = typed TS functions in `src/lib/server/tools/<domain>.ts`. One file per data source. Each tool is a small async function returning a flat JSON-friendly object. Routes call tools directly today; the agent will register them later.
...
These came out of a "what's missing for *this* family" pass — keep grouped because they share the morning-routine context.

- **NSW school term + public holiday feed** — knows it's a pupil-free day or holiday, suppresses the bus card and "morning rush" framing. Source: `date-holidays` npm or NSW Education ICS feed. Cheapest one to build first.
- **Council bin-night calendar** (City of Sydney iCal feed or per-council scrape) — "green bin tonight" is real receipt content. Belongs on the *evening* shout, not morning.
...
- [ ] `TELEGRAM_BOT_TOKEN` — @BotFather. Phase 3 (chat surfaces)
...
5. (Later, when the agent loop exists) register in `src/lib/server/agent/tools.ts`.
