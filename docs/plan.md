...
> **Plan, not spec.** This doc records intent and reasoning — chunks of it predate the code or diverged while building. The source of truth is the actual code plus `CLAUDE.md` and the folder `CLAUDE.md` files. Don't assume a section here is current; always check against actual.
...
- **Tomorrow lookahead** — ✅ *shipped, but not as planned.* The plan was one compressed `tmrw:` line on the morning brief (density-tiered acronym extraction, `compressEventTitle` helpers). In practice it became a full second print: the `evening-print` job (`15 19 * * 0-4`, Sun–Thu) screenshots each person tomorrow's *complete* brief — weather, school, events, chores. A whole sheet the night before beat cramming a line onto the morning one, so the compression idea was dropped.
