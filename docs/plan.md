...
- **Tomorrow lookahead** — ✅ *shipped, but not as planned.* The plan was one compressed `tmrw:` line on the morning brief (density-tiered acronym extraction, `compressEventTitle` helpers). In practice it became a full second print: the `evening-print` job (`15 19 * * *`) screenshots each person tomorrow's *complete* brief — weather, school, events, chores. A whole sheet the night before beat cramming a line onto the morning one, so the compression idea was dropped.
